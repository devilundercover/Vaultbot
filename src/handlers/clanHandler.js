...
module.exports = {
  ...
  async startApplication(interaction) {
    ...
    modal.addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('banned').setLabel('2. Wurdest du auf HugoSMP gebannt? (Wenn ja, warum?)').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(300)
      ),
      new ActionRowBuilder().addComponents(
